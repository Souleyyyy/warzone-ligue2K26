// ── WARZONE LEAGUE 2025 — APP (v6 - simple JSON sync) ───────────────────────
const App = (() => {
  ...
  function go(page,e) {
    ...
    curPage=page; window.scrollTo(0,0); closeMobileMenu(); render(page);
  }
  ...
  // ── RENDER HOME ────────────────────────────────────────────────────────────
  function renderHome() {
    const stats=WZ.getStats(), next=WZ.getNextJournee(), total=WZ.getTotalPartiesJouees(), logo=WZ.getLogo();
    const logoEl=document.getElementById('hero-logo-container');
    if(logoEl) logoEl.innerHTML=logo?`<img src="${logo}" alt="WZ" style="max-height:100px;max-width:280px;object-fit:contain">`:`<div class="hero-tag"><div class="hero-tag-pulse"></div>WARZONE LEAGUE 2025</div>`;
    const heroBg=document.getElementById('hero-bg');
    if(heroBg){if(logo){heroBg.className='hero-bg-media';heroBg.style.backgroundImage=`url(${logo})`;}else{heroBg.className='';heroBg.style.backgroundImage='';}}
    const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
    set('hs-parties',total);set('hs-joueurs',WZ.NAMES.length);set('hs-leader',stats[0]?.name.toUpperCase()||'—');set('hs-pts',stats[0]?.total||'0');
    ...
    if(podEl){
      ...
      podEl.innerHTML=top3.some(p=>p.total>0)
        ?top3.map((p,i)=>`<div class="podium-row ${cls[i]}"><div class="pod-medal">${medals[i]}</div>${avatar(p.name,`pod-avatar ${cls[i]}`,38)}<div><div class="pod-name">${p.name}</div><div class="pod-info">${p.kills} kills · ${p.partiesJouees} parties</div></div><div class="pod-pts">${p.total}</div></div>`).join('')
    ...
    if(ngEl){
      if(next){
        const row=(lbl,grp)=>`<div class="npart"><div class="npart-lbl">${lbl}</div><div class="npart-names">${grp.map(n=>WZ.TOP3.has(n)?`<span class="top3-name">${n}</span>`:n).join(' · ')}</div></div>`;
      ...
      } else ngEl.innerHTML=`<div style="color:var(--text2);font-size:14px;padding:14px 0">🏆 Toutes les journées sont terminées !</div>`;
    }
  }

  // ── RENDER CLASSEMENT ──────────────────────────────────────────────────────
  function renderClassement() {
    const stats=WZ.getStats(),maxPts=Math.max(1,stats[0]?.total||1),rows=document.getElementById('rank-rows');
    if(!rows)return;
    const total=WZ.getTotalPartiesJouees();
    const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
    set('cl-parties',total);set('cl-leader',stats[0]?.name||'—');set('cl-kills',stats[0]?.kills||0);set('cl-pts',stats[0]?.total||0);
    ...
    rows.innerHTML=stats.map((p,i)=>{
      ...
      return `<div class="rank-row ${rcls(i)} fu" style="animation-delay:${i*.035}s">
  ...
  function renderCalendrier() {
    ...
    grid.innerHTML=WZ.SCHEDULE.map(day=>{
      const st=WZ.getJourneeStatus(day.j),res=WZ.getState().results[day.j];
      const stTxt={done:'Terminée',partial:'En cours',pending:'À jouer'}[st];
      return `<div class="jour-card ${st} fu">
        ...
        ${[['A',day.g1],['B',day.g2],['C',day.g3]].map(([lbl,grp])=>{
  ...
  function renderJoueurs() {
    ...
    const stats=WZ.getStats(),meds={1:'🥇',2:'🥈',3:'🥉'},bds={1:'var(--gold)',2:'#c0c0cd',3:'#cd7f32'};
    grid.innerHTML=stats.map((p,i)=>{
      ...
      return `<div class="player-card ${isT3?'top3-card':''} fu" style="animation-delay:${i*.04}s">
        <div class="pc-avatar" style="border-color:${bd}">${avIn}${meds[rank]?`<div class="pc-rank-badge">${meds[rank]}</div>`:''}</div>
        <div class="pc-name">${p.name}</div><div class="pc-pts">${p.total} pts</div>
  ...
  function renderAdmin() {
    const lock=document.getElementById('admin-lock'),panel=document.getElementById('admin-panel');
  ...
  function renderTab(tab) {
    ...
    if(tab==='publier')   buildPublierForm();
  }

  // ── PUBLIER FORM ───────────────────────────────────────────────────────────
  function buildPublierForm() {
    const wrap=document.getElementById('publier-form'); if(!wrap)return;
    wrap.innerHTML=`
      <div style="max-width:560px">
        ...
        <div style="background:rgba(232,184,75,0.06);border:1px solid var(--gold-border);border-radius:14px;padding:20px 22px;margin-bottom:20px">
          <div style="font-family:var(--font-title);font-size:16px;font-weight:700;color:var(--gold);margin-bottom:8px">📡 Comment publier les résultats</div>
          <div style="font-size:14px;color:var(--text2);line-height:1.9">
            1. Saisis les kills dans <strong>"Saisie manuelle"</strong><br>
            2. Clique <strong>"⬇ Télécharger data.json"</strong> ci-dessous<br>
            3. Va sur <strong>github.com/Souleyyyy/warzone-ligue2K26</strong><br>
            4. Clique sur <strong>data.json</strong> → crayon ✏️ → <strong>Upload file</strong><br>
            &nbsp;&nbsp;&nbsp;(ou fais glisser le fichier téléchargé)<br>
            5. Clique <strong>"Commit changes"</strong><br>
            6. ✅ <strong>Tous les appareils voient les données en 1-2 min</strong>
          </div>
        </div>

        <button class="btn-primary" style="width:100%;font-size:15px;padding:16px"
          onclick="App.downloadData()">
          ⬇ Télécharger data.json
        </button>

        <div style="margin-top:12px;font-family:var(--font-mono);font-size:11px;color:var(--text3);letter-spacing:1px;text-align:center">
          // Le fichier contient tous les scores et résultats actuels
        </div>

        <div style="margin-top:24px;background:var(--glass);border:1px solid var(--border);border-radius:12px;padding:16px 18px">
          <div style="font-family:var(--font-title);font-size:13px;font-weight:700;color:var(--text);margin-bottom:10px">🔄 Les participants voient les données :</div>
          <div style="font-size:13px;color:var(--text2);line-height:1.8">
            • Automatiquement au chargement du site<br>
            • En actualisant la page (F5 ou pull-to-refresh sur mobile)<br>
            • Le site charge toujours la dernière version de <code style="color:var(--gold);background:rgba(232,184,75,0.1);padding:1px 5px;border-radius:4px">data.json</code> depuis GitHub
          </div>
        </div>
  ...
  function buildKillsForm() {
    const wrap=document.getElementById('kills-form'); if(!wrap)return;
    wrap.innerHTML=WZ.SCHEDULE.map(day=>{
      const res=WZ.getState().results[day.j];
      return `<div class="kills-day">
        ...
        ${[['A',day.g1],['B',day.g2],['C',day.g3]].map(([p,grp])=>{
          ...
          return `<div class="kills-partie ${validated?'validated':''}" id="kpart-${day.j}-${p}">
            ...
            ${pdata.map((pl,pi)=>{
              ...
              return `<div class="kp-row">
                ...
                <div class="kp-inputs">${kills.map((k,ki)=>`<input class="kp-input" type="number" min="0" max="99" value="${k}" onchange="App.setKills(${day.j},'${p}',${pi},${ki},this.value)">`).join('')}</div>
  ...
  function buildSancForm() {
    const grid=document.getElementById('sanc-grid'); if(!grid)return;
    const sancs=WZ.getSanctions();
    grid.innerHTML=WZ.NAMES.map(n=>{
      const cur=sancs[n]||0;
      return `<div class="sanc-card">
  ...
  function buildPhotosForm() {
    const wrap=document.getElementById('photos-form'); if(!wrap)return;
    ...
    wrap.innerHTML=`
      <div style="margin-bottom:28px">
        ...
        <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap">
          ...
          <label class="btn-ghost" style="cursor:pointer;font-size:13px">${logo?'↺ Changer':'+ Logo'}
      ...
      <div class="photos-grid">${WZ.NAMES.map(name=>{
        ...
        return `<div class="photo-card">
          ...
          <label class="btn-ghost" style="cursor:pointer;font-size:11px;padding:6px 12px">${photo?'↺ Changer':'+ Photo'}
  ...
  // ── ACTIONS ────────────────────────────────────────────────────────────────
  function tryLogin() {
    const pw=document.getElementById('admin-pw'); if(!pw)return;
    if(pw.value===PASS){adminOk=true;sessionStorage.setItem('wz_ok','1');renderAdmin();toast('Accès accordé ✓','ok');}
    else{const err=document.getElementById('login-err');if(err)err.style.display='block';pw.value='';}
  }
  ...
  function setKills(j,p,pi,ki,val) {
    const r=WZ.getState().results[j]?.[p]; if(!r)return;
    const kills=[...(r.players[pi]?.kills||[0,0,0,0,0])];
    kills[ki]=parseInt(val)||0; WZ.setKills(j,p,pi,kills);
  ...
  function addSanc(name) {
    ...
    WZ.addSanction(name,pts);buildSancForm();
    if(curPage==='classement')renderClassement();
  ...
  function clearSanc(name) {
    ...
    if(curPage==='classement')renderClassement();toast(`Sanctions ${name} réinitialisées`,'ok');
  }
